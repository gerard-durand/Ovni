...
mingw32-make -f makefile.gcc -j 10 SHARED=1 MONOLITHIC=1 BUILD=release VENDOR=%CB_ID% CXXFLAGS="-std=gnu++26"
rem >log.txt 2>&1
echo %time% >> 00_test.txt
strip ..\..\lib\gcc_dll\wxmsw332u_gcc_%CB_ID%.dll
strip ..\..\lib\gcc_dll\wxmsw332u_gl_gcc_%CB_ID%.dll
echo %time% >> 00_test.txt
