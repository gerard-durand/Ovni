...
int Load_TGA(const char *name, unsigned char **buffer, int *width, int *height) {
...
//    glTexImage2D (GL_TEXTURE_2D,  0, 4, iw, ih, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, *buffer); // GL_BGRA_EXT plutôt que GL_RGBA
    gluBuild2DMipmaps(GL_TEXTURE_2D, 4, iw, ih,    GL_BGRA_EXT, GL_UNSIGNED_BYTE, *buffer); // GL_BGRA_EXT plutôt que GL_RGBA
    free(data);
//    return 1;
    return texture_id;
